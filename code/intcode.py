...
class Computer():
    ...
    def process_instruction(self, execute_until_input = False):
        cursor = self.cursor
        start_cursor = cursor
        ...
        if opcode == 1:
            ...
            if len(self.inputs) == 0:
                ...
                self.sequence_dict[target_position] = self.inputs.pop(0)

                if execute_until_input == True:
    ...
    def execute_until_output_multiple_values(self, num_outputs):
        outputs = []
        while True:
            output = self.process_instruction()
            if output is not None:
                if output == "Awaiting Input":
    ...
    def execute_until_input(self):
        while True:
            output = self.process_instruction(True)
            if output is not None:
                return output
